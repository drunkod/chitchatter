// Target: src/components/VisualNovelRoom/VisualNovelRoom.tsx
//
// Auto-start: as soon as the session finishes syncing and no story state
// exists for the room, the first bundled story starts automatically —
// no "Start story" button. A ref prevents duplicate startStory() calls
// (StrictMode / rapid snapshot updates). Late joiners never trigger it
// because they receive `snapshot.state` from the controller during sync.
...
export const VisualNovelRoom = ({ peerRoom }: VisualNovelRoomProps) => {
  ...
  if (!snapshot.state) {
    // Auto-start is in progress — show a lightweight status instead of the
    // old "Start story" panel. (Kept minimal; no button needed.)
    return (
      <Paper
        component="section"
        aria-label="Novella"
