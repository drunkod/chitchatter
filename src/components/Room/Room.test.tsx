...
describe('Room', () => {
  beforeEach(() => {
    mockMessagedSender.mockReset()
    mockMessagedSender.mockResolvedValue([])
  })
  ...
  test('re-enables sending and removes the optimistic message after a send failure', async () => {
    const consoleError = vi.spyOn(console, 'error').mockImplementation(() => {})

    mockMessagedSender.mockRejectedValueOnce(new Error('Network failure'))

    render(
      <RouteStub>
        <RoomStub
          getUuid={mockGetUuid.mockImplementation(() => 'failed-message')}
          userId={mockUserId}
          roomId={mockRoomId}
        />
      </RouteStub>
