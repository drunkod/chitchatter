...
const transition = (
  ...
  if (input.sceneId !== undefined && !isId(input.sceneId)) {
    errors.push(`${path}.sceneId is invalid`)
  }
  if (input.dialogueEntryId !== undefined && !isId(input.dialogueEntryId)) {
    errors.push(`${path}.dialogueEntryId is invalid`)
  }
...
const entry = (
  ...
  if (input.portrait !== undefined && !isId(input.portrait)) {
    errors.push(`${path}.portrait is invalid`)
  }
  if (input.soundEffect !== undefined && !isId(input.soundEffect)) {
    errors.push(`${path}.soundEffect is invalid`)
  }
  ...
  if (
    ...
    !Array.isArray(input.characterChanges)
  ) {
    errors.push(`${path}.characterChanges must be an array`)
  }
...
const scene = (
  ...
  if (input.background !== undefined && !isId(input.background)) {
    errors.push(`${path}.background is invalid`)
  }
  if (input.music !== undefined && !isId(input.music)) {
    errors.push(`${path}.music is invalid`)
  }
  if (input.characters !== undefined && !Array.isArray(input.characters)) {
...
export const validateStory = (
  ...
  if (
    ...
    errors.push('Invalid story.description')
  }
    ...
    currentScene.dialogue.forEach(currentEntry => {
      ...
      if (currentEntry.next) {
        ...
        if (!targetScene) {
          ...
          errors.push(`Entry ${currentEntry.id} targets unknown dialogue entry`)
        }
      }
    })
  })
